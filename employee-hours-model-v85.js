(function (root) {
  ...
  function change(s,action){
    ...
    switch(action.type){
      ...
      case 'status':if(!n.employees.some(e=>e.id===action.id)||!['present','absent'].includes(action.value))throw Error('Zaposleni nije pronađen.');ensureDay(n).statuses[action.id]=action.value;if(action.value==='absent'&&ensureDay(n).overrides)delete ensureDay(n).overrides[action.id];break
      case 'all':if(!['present','absent'].includes(action.value))throw Error('Neispravan status.');for(const e of n.employees){ensureDay(n).statuses[e.id]=action.value;if(action.value==='absent'&&ensureDay(n).overrides)delete ensureDay(n).overrides[e.id]}break
      case 'cell':{
        if(!validDate(action.date)||!n.employees.some(e=>e.id===action.id))throw Error('Neispravan dan ili zaposleni.')
      ...
      case 'remove':n.employees=n.employees.filter(e=>e.id!==action.id);for(const d of Object.values(n.days)){delete d.statuses[action.id];if(d.overrides)delete d.overrides[action.id]};break
  ...
  function month(s){
    ...
    const rows=s.employees.map(e=>{const cells=dates.map(({date})=>{const d=s.days[date],status=d?.statuses[e.id];return status==='present'?(d.overrides?.[e.id]??d.minutes):status==='absent'?0:null});return {employee:e,cells,total:cells.reduce((a,b)=>a+(b||0),0)}})
