(function(root){
  ...
  document.addEventListener('tasker-hours-export',async event=>{
    ...
    try{
      ...
      const site=(s.site||'Gradiliste').replace(/[^\p{L}\p{N}-]+/gu,'-').slice(0,65),file=new File([blob],`Radni-sati-${s.month}-${site}${personName?'-'+personName.replace(/[^\p{L}\p{N}-]+/gu,'-'):''}-v91.${type}`,{type:blob.type})
