...
async function receiptProjectHome(view='overview') {
  ...
  content.querySelectorAll('[data-receipt-image]').forEach((button)=>button.addEventListener('click',async()=>{const blob=await getReceiptImage(button.dataset.receiptImage);if(!blob)return;const url=URL.createObjectURL(blob);const modal=document.createElement('div');modal.className='receipt-image-modal';modal.innerHTML=`<section><button type="button">×</button><img src="${url}" alt="Fotografija računa"></section>`;const close=()=>{URL.revokeObjectURL(url);modal.remove()};modal.addEventListener('click',(event)=>{if(event.target===modal||event.target.closest('button'))close()});document.body.appendChild(modal)}))
  content.querySelectorAll('[data-receipt-delete]').forEach((button)=>button.addEventListener('click',async()=>{if(!confirm('Obrisati ovaj račun?'))return;await deleteReceiptImage(button.dataset.receiptDelete);taskerReceipts=taskerReceipts.filter((item)=>item.id!==button.dataset.receiptDelete);saveTaskerReceipts();receiptProjectHome('table')}))
}
