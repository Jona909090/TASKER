...
async function receiptProjectHome(view='overview') {
  ...
  root.querySelector('#receipt-photo')?.addEventListener('change',(event)=>{const file=event.target.files?.[0],preview=root.querySelector('#receipt-photo-preview');if(!file||!preview)return;preview.hidden=false;preview.querySelector('img').src=URL.createObjectURL(file)})
