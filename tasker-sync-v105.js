(function(root){
  ...
  function mount(){
    for(const [d,c]of Object.entries(configs)){
      ...
      if(d==='modules'){
        const toolbar=host.querySelector('.ms-header .ms-actions')
        const dialog=document.createElement('dialog');dialog.id='ts-modules-dialog';dialog.className='ts-dialog';dialog.setAttribute('aria-label','Povezivanje tableta i računara')
        const close=document.createElement('button');close.type='button';close.className='ts-close';close.textContent='×';close.setAttribute('aria-label','Zatvori povezivanje');close.onclick=()=>dialog.close()
        dialog.append(close,panel);host.append(dialog)
        const button=document.createElement('button');button.type='button';button.id='ts-modules-open';button.className='ms-button';button.textContent='📁 Povezivanje';button.setAttribute('aria-haspopup','dialog');button.setAttribute('aria-controls',dialog.id);button.onclick=()=>dialog.showModal()
        if(toolbar)toolbar.append(button)
        dialog.addEventListener('close',()=>button.focus())
      }else host.prepend(panel)
      message(d,localStorage.getItem(TOKEN)?(d==='modules'?'Spremno. Izaberite slanje ili preuzimanje.':'Veza je podešena. Provera u toku…'):'Nije povezano. Podaci ostaju na ovom uređaju.')
    }
  }
