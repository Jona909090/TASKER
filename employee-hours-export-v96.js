(function(root){
  ...
  const api={workbook,paintPdf,color,monthName,excelFormat}
  ...
  document.addEventListener('tasker-hours-export',async event=>{
    ...
    try{
      const {state:source,action,format,employeeId}=event.detail
      ...
      const site=(s.site||'Gradiliste').replace(/[^\p{L}\p{N}-]+/gu,'-').slice(0,65),file=new File([blob],`Radni-sati-${s.month}-${site}${personName?'-'+personName.replace(/[^\p{L}\p{N}-]+/gu,'-'):''}-v96-${Date.now()}.${type}`,{type:blob.type})
      prepared={file,channel:action,subject:`Radni sati${personName?' – '+personName:''} – ${s.site||'Gradilište'} – ${monthName(s)}`,text:`Mjesečna evidencija radnih sati${personName?' – '+personName:''}\n${s.site||'Gradilište'}\n${monthName(s)}\nUkupno svih sati: ${number(m.total)} h`}
