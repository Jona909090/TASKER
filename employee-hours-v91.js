...
(function () {
  ...
  function exportState(){
    ...
    for(const d of Object.values(copy.days)){for(const key of ['statuses','overrides'])if(d[key])d[key]=Object.fromEntries(Object.entries(d[key]).filter(([id])=>ids.has(id)))}
    return copy
  }
  ...
  document.addEventListener('click',event=>{
    ...
    if((b=target.closest('[data-eh-export]'))){if(!exportState().employees.length)return;document.dispatchEvent(new CustomEvent('tasker-hours-export',{detail:{state:exportState(),employeeId:selectedEmployeeId,action:b.dataset.ehExport,format:el('eh-export-format').value}}));return}
