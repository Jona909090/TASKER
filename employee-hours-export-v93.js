(function(root){
  ...
  function workbook(ExcelJS,s,m,existing=null,sheetName='Radni sati'){
  ...
  function siteWorkbook(ExcelJS,s,M,siteId=''){
    const w=new ExcelJS.Workbook(),used=new Set()
    const groups=siteId?[{id:siteId,name:siteId==='unassigned'?'Neraspoređeno':M.sites(s).find(x=>x.id===siteId)?.name}]:M.sites(s).map(x=>({...x}))
    if(!siteId&&M.month(M.siteState(s,'unassigned')).rows.length)groups.push({id:'unassigned',name:'Neraspoređeno'})
    for(const group of groups){
      const filtered=M.siteState(s,group.id),base=String(group.name||'Gradilište').replace(/[\\\\/?*\[\]:]/g,' ').replace(/^'+|'+$/g,'').trim().slice(0,31)||'Gradilište'
      let name=base,i=2
      while(used.has(name.toLocaleLowerCase())){const suffix=' ('+(i++)+')';name=base.slice(0,31-suffix.length)+suffix}
      used.add(name.toLocaleLowerCase())
      workbook(ExcelJS,filtered,M.month(filtered),w,name)
    }
    return w
  }
  ...
  const api={workbook,paintPdf,color,monthName,excelFormat,siteWorkbook}
  ...
  document.addEventListener('tasker-hours-export',async event=>{
    ...
    try{
      const {state:source,action,format,employeeId,siteId}=event.detail
      const s=JSON.parse(JSON.stringify(source))
      if(employeeId)s.employees=s.employees.filter(e=>e.id===employeeId)
      const display=root.TaskerHoursModel.siteState(s,siteId||'')
      if(!siteId)display.site='SVA GRADILIŠTA'+(s.site?' – '+s.site:'')
      const m=root.TaskerHoursModel.month(display),person=employeeId?m.rows[0]?.employee:null
      ...
      let blob
      if(type==='xlsx'){const Excel=await library('excel','https://cdnjs.cloudflare.com/ajax/libs/exceljs/4.4.0/exceljs.min.js',()=>root.ExcelJS);const w=siteWorkbook(Excel,s,root.TaskerHoursModel,siteId||'');blob=new Blob([await w.xlsx.writeBuffer()],{type:mimeXlsx})}
      ...
      const site=(siteId?display.site:'Sva-gradilista').replace(/[^\p{L}\p{N}-]+/gu,'-').slice(0,65),file=new File([blob],`Radni-sati-${s.month}-${site}${personName?'-'+personName.replace(/[^\p{L}\p{N}-]+/gu,'-'):''}-v93.${type}`,{type:blob.type})
      prepared={file,channel:action,subject:`Radni sati${personName?' – '+personName:''} – ${display.site} – ${monthName(s)}`,text:`Mjesečna evidencija radnih sati${personName?' – '+personName:''}\n${display.site}\n${monthName(s)}\nUkupno svih sati: ${number(m.total)} h`}
