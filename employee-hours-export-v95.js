(function(root){
  ...
  function siteWorkbook(ExcelJS,s,M,siteId=''){
    ...
    if(siteId==='unassigned')throw Error('Odaberite gradilište za Excel izvoz.')
  ...
  document.addEventListener('tasker-hours-export',async event=>{
    ...
    try{
      ...
      const site=(siteId?display.site:'Sva-gradilista').replace(/[^\p{L}\p{N}-]+/gu,'-').slice(0,65),file=new File([blob],`Radni-sati-${s.month}-${site}${personName?'-'+personName.replace(/[^\p{L}\p{N}-]+/gu,'-'):''}-v95-${Date.now()}.${type}`,{type:blob.type})
