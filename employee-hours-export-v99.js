(function(root){
  ...
  document.addEventListener('tasker-hours-export',async event=>{
    ...
    try{
      ...
      const site=(multiple?'Sva-gradilista':s.site||'Gradiliste').replace(/[^\p{L}\p{N}-]+/gu,'-').slice(0,65),file=new File([blob],`Radni-sati-${s.month}-${site}${personName?'-'+personName.replace(/[^\p{L}\p{N}-]+/gu,'-'):''}-v99-${Date.now()}.${type}`,{type:blob.type})
      ...
      showReady()
      if(action==='excel'||action==='pdf'){status('Dokument je spreman. Kliknite Preuzmi dokument ispod.');try{download(file)}catch{status('Automatsko preuzimanje nije dostupno. Kliknite Preuzmi dokument ispod.')}}else{modal();status('Dokument je spreman za dijeljenje.')}
