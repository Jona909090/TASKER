...
(function () {
  ...
  const B=window.TaskerHoursBooks,M=window.TaskerHoursModel, KEY='tasker.employee-hours.v1'
  let state,ledger,activeBookId='base', loadError=''
  let employeeQuery='', selectedEmployeeId=''
  const normalize=s=>s.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLocaleLowerCase().trim()
  function exportState(source=state){
    const copy=JSON.parse(JSON.stringify(source));delete copy.hourBooks
  ...
  try {const raw=localStorage.getItem(KEY);ledger=raw?B.validate(JSON.parse(raw),M):M.create();state=B.records(ledger)[0].state} catch(e){loadError=e.message}
  ...
  function commit(action, redraw=true){
    try {const left=el('eh-table')?.scrollLeft||0,top=el('eh-table')?.scrollTop||0;const raw=localStorage.getItem(KEY);const latest=raw?B.validate(JSON.parse(raw),M):ledger;const next=B.change(latest,activeBookId,action,M);localStorage.setItem(KEY,JSON.stringify(next));ledger=next;state=B.records(ledger).find(b=>b.id===activeBookId).state;if(redraw){refresh();if(el('eh-table')){el('eh-table').scrollLeft=left;el('eh-table').scrollTop=top}}renderBooks();el('eh-save').textContent='Sačuvano na ovom uređaju';return true}
  ...
  `
  extra.textContent+=`
    #eh-book-editor{width:min(480px,calc(100% - 28px));box-sizing:border-box;background:#152b43;color:#e8f6ff;border:1px solid #3c7c97;border-radius:16px;padding:24px}
    #eh-book-editor::backdrop{background:#020b16bb}#eh-book-editor label{margin:14px 0}
    #employee-hours-project #eh-excel-scope{max-width:100%;padding:10px;background:#10273e;color:#fff;border:1px solid #3a6683;border-radius:8px}
  `
  document.head.append(extra)
  function renderBooks(){
    if(!el('eh-book-tabs'))return
    el('eh-book-tabs').innerHTML=B.records(ledger).map(b=>`<button class="eh-btn ${b.id===activeBookId?'chosen':''}" data-eh-book="${esc(b.id)}" aria-pressed="${b.id===activeBookId}">${esc(b.state.site||'Postojeće gradilište')}</button>`).join('')
    const book=B.records(ledger).find(b=>b.id===activeBookId)
    el('eh-book-info').textContent=book.start?'Evidencija ovog gradilišta počinje '+book.start.split('-').reverse().join('.')+'. Raniji dani ostaju bez sati.':'Postojeća evidencija — raniji sati ostaju sačuvani.'
  }
  function open () {
    ...
    content.innerHTML=`<section id="${pageId}">
      ...
      <section class="eh-panel"><div class="eh-month-head"><h2>Gradilišta</h2><button class="eh-btn primary" id="eh-add-book">+ Dodaj gradilište</button></div><div id="eh-book-tabs" class="eh-tools"></div><p id="eh-book-info"></p></section>
      ...
      <section class="eh-panel"><div class="eh-month-head"><h2>Prisutnost zaposlenih</h2><button class="eh-btn" id="eh-add">+ DODAJ ZAPOSLENOG</button></div><div class="eh-tools"><button class="eh-btn primary" data-eh-all="present">OZNAČI SVE PRISUTNE</button><button class="eh-btn" data-eh-all="absent">OZNAČI SVE ODSUTNE</button></div><p id="eh-unmarked"></p><div id="eh-people"></div><details open><summary>Odjavljeni zaposleni — ispravak odjave</summary><div id="eh-former"></div></details></section>
      <section class="eh-panel" id="eh-month-panel"><div class="eh-month-head"><h2>MJESEČNA EVIDENCIJA SATI</h2><button class="eh-btn" id="eh-expand" aria-pressed="false">⛶ Proširi tablicu</button></div><div class="eh-fields"><label>Projekt / Gradilište<input id="eh-site" placeholder="npr. VERTIV" value="${esc(state.site)}"></label><label>Mjesec i godina<input id="eh-month" type="month" value="${state.month}"></label></div><div class="eh-fields"><label>Pronađi zaposlenog za zasebnu tablicu<input id="eh-employee-search" type="search" autocomplete="off" placeholder="Upiši ime ili prezime"></label><div class="eh-tools"><button type="button" id="eh-show-all" class="eh-btn">Svi zaposleni</button></div></div><div id="eh-search-results" class="eh-tools" aria-live="polite"></div><p id="eh-export-scope" role="status"></p><div id="eh-month-title" class="eh-month-title"></div><p>Žuto = odrađeni sati · – = bez sati. Kliknite ćeliju i upišite sate; Enter ili izlazak iz ćelije automatski sprema izmjenu. Ručni sati ostaju sačuvani pri promjeni zajedničkih sati.</p><div id="eh-table" class="eh-scroll" role="region" aria-label="Mjesečna evidencija sati" tabindex="0"></div><div id="eh-month-total" class="eh-month-total"></div><div class="eh-tools"><label>Excel — gradilišta<select id="eh-excel-scope"><option value="all" selected>Sva gradilišta — zasebne kartice</option><option value="current">Samo trenutno gradilište</option></select></label><button type="button" class="eh-btn" data-eh-export="excel">Preuzmi Excel</button><button type="button" class="eh-btn" data-eh-export="pdf">Preuzmi PDF</button><label>Format za slanje<select id="eh-export-format"><option value="pdf">PDF</option><option value="xlsx">Excel</option></select></label><button type="button" class="eh-btn primary" data-eh-export="whatsapp">WhatsApp</button><button type="button" class="eh-btn" data-eh-export="email">E-mail</button></div><p id="eh-export-status" role="status">Excel automatski sadrži sva gradilišta za izabrani mjesec, svako na zasebnoj kartici (i ona bez sati). PDF: trenutno gradilište. Nakon izmjena ponovno preuzmite dokument.</p></section>
      <dialog id="eh-book-editor"><form id="eh-book-form"><h2>Dodaj gradilište</h2><label>Naziv gradilišta<input id="eh-book-name" required maxlength="100" placeholder="npr. MONTING / VERTIV"></label><label>Početak evidencije<input id="eh-book-start" type="date" required></label><p>Preuzima se popis zaposlenih, bez kopiranja ranijih sati.</p><p id="eh-book-error" role="alert"></p><div class="eh-tools"><button type="button" class="eh-btn" id="eh-book-cancel">Odustani</button><button type="submit" class="eh-btn primary">Dodaj gradilište</button></div></form></dialog>
  ...
  function refresh(){
    ...
    el('eh-people').innerHTML=people.length?people.map((e,i)=>{
      ...
      return `<article class="eh-person"><div class="eh-identity"><span class="eh-avatar">${esc((e.first[0]+e.last[0]).toLocaleUpperCase())}</span><div><strong>${esc(e.first)} ${esc(e.last)}</strong><small>${esc(e.role)} · ${hours}</small></div></div><div class="eh-person-actions"><button class="eh-btn ${status==='present'?'is-present':''}" data-eh-status="present" data-eh-id="${esc(e.id)}" aria-pressed="${status==='present'}">PRISUTAN</button><button class="eh-btn ${status==='absent'?'is-absent':''}" data-eh-status="absent" data-eh-id="${esc(e.id)}" aria-pressed="${status==='absent'}">ODSUTAN</button><button class="eh-btn eh-mini" data-eh-edit="${esc(e.id)}">Uredi</button><button class="eh-btn eh-mini" data-eh-move="-1" data-eh-id="${esc(e.id)}" aria-label="Pomakni prema gore" ${i===0?'disabled':''}>↑</button><button class="eh-btn eh-mini" data-eh-move="1" data-eh-id="${esc(e.id)}" aria-label="Pomakni prema dolje" ${i===people.length-1?'disabled':''}>↓</button><button class="eh-btn eh-mini" data-eh-remove="${esc(e.id)}">Prestanak rada</button></div></article>`
    }).join(''):'<div class="eh-empty"><strong>Još nema zaposlenih</strong>Kliknite + DODAJ ZAPOSLENOG za prvi unos.</div>'
    el('eh-former').innerHTML=state.employees.filter(e=>e.endDate).map(e=>`<div class="eh-tools"><span>${esc(e.first)} ${esc(e.last)} · zadnji radni dan ${e.endDate.split('-').reverse().join('.')}.</span><button class="eh-btn" data-eh-edit="${esc(e.id)}">Promijeni datum</button><button type="button" class="eh-btn primary" data-eh-restore="${esc(e.id)}">↶ Vrati zaposlenog</button></div>`).join('')||'<p>Nema zaposlenih s evidentiranim prestankom rada.</p>'
    ...
    const scoped=exportState(),m=M.month(scoped),current=M.today(),cls=x=>(x.weekend?'eh-weekend ':'')+(x.date===current?'eh-today':'')
    ...
    document.querySelectorAll('[data-eh-export]').forEach(b=>{b.disabled=Boolean(pending)||(b.dataset.ehExport==='pdf'&&!m.rows.length)})
  ...
  document.addEventListener('click',event=>{
    ...
    if(target.closest('#eh-add-book')){el('eh-book-name').value='';el('eh-book-start').value=state.date;el('eh-book-error').textContent='';el('eh-book-editor').showModal();el('eh-book-name').focus();return}
    if(target.closest('#eh-book-cancel')){el('eh-book-editor').close();return}
    ...
    let b
    if((b=target.closest('[data-eh-book]'))){const raw=localStorage.getItem(KEY);try{if(raw)ledger=B.validate(JSON.parse(raw),M);const book=B.records(ledger).find(x=>x.id===b.dataset.ehBook);if(!book)return;activeBookId=book.id;state=book.state;employeeQuery='';selectedEmployeeId='';open()}catch(e){el('eh-save').textContent=e.message}return}
    ...
    if((b=target.closest('[data-eh-export]'))){try{
      const raw=localStorage.getItem(KEY);if(raw)ledger=B.validate(JSON.parse(raw),M)
      state=B.records(ledger).find(x=>x.id===activeBookId).state
      if(employeeQuery.trim()&&!selectedEmployeeId)return
      const books=el('eh-excel-scope').value!=='current'?B.records(ledger).map(x=>exportState({...x.state,month:state.month})):[exportState()]
      document.dispatchEvent(new CustomEvent('tasker-hours-export',{detail:{state:exportState(),books,employeeId:selectedEmployeeId,action:b.dataset.ehExport,format:el('eh-export-format').value}}))
    }catch(e){el('eh-export-status').textContent='Izvoz nije moguć: '+e.message}return}
    ...
    if((b=target.closest('[data-eh-restore]'))){
      const person=state.employees.find(e=>e.id===b.dataset.ehRestore)
      if(!person?.endDate)return
      if(!confirm('Poništiti odjavu za '+person.first+' '+person.last+'? Datum prestanka rada bit će uklonjen, a ranije uneseni sati ostaju sačuvani.'))return
      if(commit({type:'employee',...person,endDate:''}))el('eh-save').textContent=person.first+' '+person.last+' je vraćen u evidenciju. Raniji sati nisu promijenjeni.'
      return
    }
  ...
  document.addEventListener('submit',event=>{
    if(event.target.id==='eh-book-form'){
      event.preventDefault()
      try{
        const raw=localStorage.getItem(KEY),latest=raw?B.validate(JSON.parse(raw),M):ledger,id=crypto.randomUUID()
        const next=B.add(latest,id,el('eh-book-name').value,el('eh-book-start').value,M,state.employees)
        if(raw&&!localStorage.getItem(KEY+'.before-worksites-v97'))localStorage.setItem(KEY+'.before-worksites-v97',raw)
        localStorage.setItem(KEY,JSON.stringify(next));ledger=next;activeBookId=id;state=B.records(ledger).find(b=>b.id===id).state;employeeQuery='';selectedEmployeeId='';el('eh-book-editor').close();open()
      }catch(e){el('eh-book-error').textContent='Nije sačuvano: '+e.message}
      return
    }
