...
(function () {
  ...
  function refresh(){
    ...
    document.querySelectorAll('[data-eh-export]').forEach(b=>{b.disabled=!m.rows.length})
  ...
  document.addEventListener('click',event=>{
    ...
    if((b=target.closest('[data-eh-export]'))){try{
      ...
      if(!exportState().employees.length)return
      const books=el('eh-excel-scope').value==='all'?B.records(ledger).map(x=>({id:x.id,state:exportState({...x.state,month:state.month})})).filter(x=>x.id===activeBookId||M.month(x.state).total>0).map(x=>x.state):[exportState()]
