...
(function () {
  ...
  function open () {
    ...
    content.innerHTML=`<section id="${pageId}">
      ...
      <section class="eh-panel" id="eh-month-panel"><div class="eh-month-head"><h2>MJESEČNA EVIDENCIJA SATI</h2><button class="eh-btn" id="eh-expand" aria-pressed="false">⛶ Proširi tablicu</button></div><div class="eh-fields"><label>Projekt / Gradilište<input id="eh-site" placeholder="npr. VERTIV" value="${esc(state.site)}"></label><label>Mjesec i godina<input id="eh-month" type="month" value="${state.month}"></label></div><div id="eh-month-title" class="eh-month-title"></div><p>Žuto = odrađeni sati · – = bez sati. Kliknite ćeliju i upišite sate; Enter ili izlazak iz ćelije automatski sprema izmjenu. Ručni sati ostaju sačuvani pri promjeni zajedničkih sati.</p><div id="eh-table" class="eh-scroll" role="region" aria-label="Mjesečna evidencija sati" tabindex="0"></div><div id="eh-month-total" class="eh-month-total"></div></section>
