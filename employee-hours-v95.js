...
(function () {
  ...
  function siteOptions(value='',all=false){return (all?'<option value="">Sva gradilišta</option>':'<option value="">Odaberi gradilište</option>')+M.sites(state).map(x=>`<option value="${esc(x.id)}" ${x.id===value?'selected':''}>${esc(x.name)}</option>`).join('')}
