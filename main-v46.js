...
async function receiptProjectHome(view='overview') {
  ...
  if(view==='table') root.innerHTML=`<section class="receipt-section-heading"><div><p class="eyebrow">ZBIRNA EVIDENCIJA</p><h2>Svi računi</h2><p>Datum, prodavnica, iznos i fotografija računa.</p></div><strong>${receiptMoney(summary.total)}</strong></section><section class="receipt-table"><header><span>Datum</span><span>Prodavnica</span><span>Iznos</span><span>Napomena</span><span>Dokument</span></header><div>${receiptRows()}</div><footer><span>UKUPNO</span><b>${receiptMoney(summary.total)}</b></footer></section>`
