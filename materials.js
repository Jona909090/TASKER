...
export function renderMaterials() {
  return `<section class="page-heading"><div><p class="eyebrow">Magacin</p><h1>Materijal</h1><p>Izaberite kategoriju za pregled artikala i stanja lagera.</p></div><button class="primary-btn add-material">＋ Dodaj materijal</button></section><label class="search-field"><span>⌕</span><input id="category-search" type="search" placeholder="Pretraži kategoriju materijala…" /></label><section class="category-grid" id="category-grid">${categories.map((category) => { const data = summary(category.id); return `<button class="category-card" data-category="${category.id}"><span class="category-icon">${category.icon}</span><span class="category-content"><strong>${category.name}</strong><small>${data.items.length} artikla</small></span><span class="category-stock"><b>${quantity(data.stock)}</b><small>${category.unit}</small></span><span class="arrow">›</span></button>` }).join('')}</section>`
}
...
export function renderCategory(categoryId) {
  ...
  return `<section class="page-heading"><div><button class="back-btn" data-back>← Sve kategorije</button><p class="eyebrow">Materijal / ${category.name}</p><h1>${category.name}</h1><p>${data.items.length} artikla · ukupno ${quantity(data.stock)} ${category.unit}</p></div><button class="primary-btn add-material">＋ Dodaj artikal</button></section><label class="search-field"><span>⌕</span><input id="material-search" type="search" placeholder="Pretraži po nazivu, standardu ili lokaciji…" /></label><section id="inventory-list" class="inventory-list">${renderItems(data.items)}</section>`
}
