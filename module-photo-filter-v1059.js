(function(){
  'use strict'
  const UNKNOWN='Neodređeno',DB='tasker-module-photos-v1'
  ...
  async function zip(rows,progress=()=>{}){
    ...
    const large=rows.length>=65535||rows.reduce((n,r)=>n+r.file.size+1024,0)>=0xffffffff
    const body=[],directory=[],enc=new TextEncoder();let offset=0,index=0
    for(const r of rows){
      let running=0xffffffff
      for(let start=0;start<r.file.size;start+=2097152){const chunk=new Uint8Array(await r.file.slice(start,start+2097152).arrayBuffer());for(const b of chunk)running=table[(running^b)&255]^(running>>>8);progress(index,rows.length)}
      const bytes={length:r.file.size},name=enc.encode(safe(r.group)+'/'+String(++index).padStart(3,'0')+'-'+safe(r.name)),checksum=(running^0xffffffff)>>>0
      const head=new Uint8Array(30+name.length+(large?20:0)),h=new DataView(head.buffer)
      ...
      const central=new Uint8Array(46+name.length+(large?28:0)),c=new DataView(central.buffer)
      ...
      if(large){
        h.setUint16(4,45,true);h.setUint32(18,0xffffffff,true);h.setUint32(22,0xffffffff,true);h.setUint16(28,20,true);
        const a=30+name.length;h.setUint16(a,1,true);h.setUint16(a+2,16,true);h.setBigUint64(a+4,BigInt(r.file.size),true);h.setBigUint64(a+12,BigInt(r.file.size),true);
        c.setUint16(4,45,true);c.setUint16(6,45,true);c.setUint32(20,0xffffffff,true);c.setUint32(24,0xffffffff,true);c.setUint32(42,0xffffffff,true);c.setUint16(30,28,true);
        const b=46+name.length;c.setUint16(b,1,true);c.setUint16(b+2,24,true);c.setBigUint64(b+4,BigInt(r.file.size),true);c.setBigUint64(b+12,BigInt(r.file.size),true);c.setBigUint64(b+20,BigInt(offset),true)
      }
    ...
    const tail=[]
    if(large){
      const z=new Uint8Array(56),v=new DataView(z.buffer);v.setUint32(0,0x06064b50,true);v.setBigUint64(4,44n,true);v.setUint16(12,45,true);v.setUint16(14,45,true);v.setBigUint64(24,BigInt(rows.length),true);v.setBigUint64(32,BigInt(rows.length),true);v.setBigUint64(40,BigInt(size),true);v.setBigUint64(48,BigInt(offset),true);
      const l=new Uint8Array(20),lv=new DataView(l.buffer);lv.setUint32(0,0x07064b50,true);lv.setBigUint64(8,BigInt(offset+size),true);lv.setUint32(16,1,true);tail.push(z,l);
      e.setUint16(8,65535,true);e.setUint16(10,65535,true);e.setUint32(12,0xffffffff,true);e.setUint32(16,0xffffffff,true)
    }
