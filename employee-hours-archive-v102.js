(function(root){
  ...
  async function remove(id){
    if(typeof id!=='string'||!id.trim())throw Error('Odaberite dokument za brisanje.')
    await transaction('readwrite',store=>store.delete(id))
    if(typeof document==='object')await render()
  }
  const api={save,list,importFile,remove};root.TaskerHoursArchive=api
  ...
  async function render(){
    ...
    try{
      ...
      el('eh-archive-list').innerHTML=records.map(r=>{
        ...
        return `<article class="eh-archive-item"><strong>${r.type==='application/pdf'?'PDF':'EXCEL'} · ${esc(r.name)}</strong><p>✓ Sačuvano na ovom uređaju · ${new Date(r.created).toLocaleString('hr-HR')} · ${Math.ceil(r.size/1024)} KB</p><div class="eh-tools"><a class="eh-btn" href="${url}" ${r.type==='application/pdf'?'target="_blank" rel="noopener"':'download="'+esc(r.name)+'"'}>${r.type==='application/pdf'?'Otvori PDF':'Otvori / preuzmi Excel'}</a><a class="eh-btn" href="${url}" download="${esc(r.name)}">Preuzmi dokument</a><button class="eh-btn primary" data-hours-archive-share="${r.id}" data-channel="WhatsApp">WhatsApp — dokument</button><button class="eh-btn" data-hours-archive-share="${r.id}" data-channel="e-mail">E-mail — dokument</button><button type="button" class="eh-btn eh-delete-document" data-hours-archive-delete="${r.id}">Obriši</button></div><p id="eh-archive-result-${r.id}" role="status"></p></article>`
  ...
  document.addEventListener('click',async event=>{
    ...
    const deleteButton=event.target.closest('[data-hours-archive-delete]')
    if(deleteButton){
      const record=entries.get(deleteButton.dataset.hoursArchiveDelete);if(!record)return
      if(!confirm('Trajno obrisati dokument „'+record.name+'” iz Tasker arhive? Ova radnja se ne može poništiti. Datoteka u Preuzimanjima i uneseni radni sati ostaju netaknuti.'))return
      deleteButton.disabled=true
      try{await remove(record.id);if(el('eh-import-status'))el('eh-import-status').textContent='Dokument „'+record.name+'” obrisan je samo iz arhive. Možete ga ponovno dodati ako imate kopiju na uređaju.'}
      catch(e){if(el('eh-archive-result-'+record.id))el('eh-archive-result-'+record.id).textContent='Brisanje nije uspjelo: '+e.message;deleteButton.disabled=false}
      return
    }
  ...
  style.textContent+='#employee-hours-project .eh-delete-document{background:#712a3a;border-color:#ee778c;color:#ffe0e4}'
