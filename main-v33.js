...
const layoutTools=(group)=>`<div class="dashboard-card-tools" data-layout-tools><span role="button" tabindex="0" data-card-move="-1" data-layout-group="${group}" title="Pomeri levo">←</span><span role="button" tabindex="0" data-card-move="1" data-layout-group="${group}" title="Pomeri desno">→</span><span role="button" tabindex="0" data-card-size="-1" data-layout-group="${group}" title="Smanji">−</span><span role="button" tabindex="0" data-card-size="1" data-layout-group="${group}" title="Povećaj">＋</span></div>`
const bindDashboardLayoutEditor=()=>{const main=document.querySelector('.dashboard-grid'),stats=document.querySelector('.stat-grid'),live=document.querySelector('.dashboard-live-strip'),search=document.querySelector('.tasker-smart-search'),toggle=document.querySelector('#dashboard-layout-toggle');if(!main||!stats||!live||!search||!toggle)return;const statIds=defaultDashboardLayout.statOrder,liveIds=defaultDashboardLayout.liveOrder;Array.from(stats.children).forEach((card,index)=>{card.dataset.layoutId=statIds[index];card.dataset.layoutGroup='stat';card.insertAdjacentHTML('beforeend',layoutTools('stat'))});Array.from(live.children).forEach((card,index)=>{card.dataset.layoutId=liveIds[index];card.dataset.layoutGroup='live';card.insertAdjacentHTML('beforeend',layoutTools('live'))});main.querySelectorAll('[data-dashboard-card]').forEach((card)=>{card.dataset.layoutId=card.dataset.dashboardCard;card.dataset.layoutGroup='main';if(!card.querySelector('[data-layout-tools]'))card.insertAdjacentHTML('afterbegin',layoutTools('main'))});search.insertAdjacentHTML('beforeend',`<div class="dashboard-section-tools" data-layout-tools><span role="button" tabindex="0" data-section-move="-1" title="Pomeri pretragu gore">↑</span><span role="button" tabindex="0" data-section-move="1" title="Pomeri pretragu dole">↓</span><span role="button" tabindex="0" data-search-size="-1" title="Smanji pretragu">−</span><span role="button" tabindex="0" data-search-size="1" title="Povećaj pretragu">＋</span></div>`);applyDashboardLayout();toggle.addEventListener('click',()=>{content.classList.toggle('dashboard-layout-editing');toggle.classList.toggle('active');toggle.textContent=content.classList.contains('dashboard-layout-editing')?'✓ Završi raspored':'↔ Rasporedi kartice'});if(content._taskerLayoutHandler)content.removeEventListener('click',content._taskerLayoutHandler,true);content._taskerLayoutHandler=(event)=>{const control=event.target.closest('[data-card-move],[data-card-size],[data-section-move],[data-search-size]');if(!control||!content.classList.contains('dashboard-layout-editing'))return;event.preventDefault();event.stopImmediatePropagation();const layout=readDashboardLayout();if(control.dataset.sectionMove){const index=layout.sections.indexOf('search'),target=index+Number(control.dataset.sectionMove);if(target>=0&&target<layout.sections.length)[layout.sections[index],layout.sections[target]]=[layout.sections[target],layout.sections[index]]}else if(control.dataset.searchSize){const widths=[50,70,85,100],current=Math.max(0,widths.indexOf(layout.searchWidth)),target=Math.max(0,Math.min(widths.length-1,current+Number(control.dataset.searchSize)));layout.searchWidth=widths[target]}else{const card=control.closest('[data-layout-id]'),id=card?.dataset.layoutId,group=control.dataset.layoutGroup;if(!id)return;const orderKey=group==='stat'?'statOrder':group==='live'?'liveOrder':'order';if(control.dataset.cardMove){const index=layout[orderKey].indexOf(id),target=index+Number(control.dataset.cardMove);if(target>=0&&target<layout[orderKey].length)[layout[orderKey][index],layout[orderKey][target]]=[layout[orderKey][target],layout[orderKey][index]]}else{const sizes=[2,3,4,6,8,12],current=Math.max(0,sizes.indexOf(Number(layout.sizes[id])||4)),target=Math.max(0,Math.min(sizes.length-1,current+Number(control.dataset.cardSize)));layout.sizes[id]=sizes[target]}}saveDashboardLayout(layout);applyDashboardLayout()};content.addEventListener('click',content._taskerLayoutHandler,true)}
