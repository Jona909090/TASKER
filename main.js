...
async function receiptProjectHome(view='overview') {
  ...
  root.querySelector('#receipt-form')?.addEventListener('submit',async(event)=>{event.preventDefault();const form=new FormData(event.currentTarget),file=form.get('photo');if(!(file instanceof Blob)||!file.size)return;const id=`receipt-${Date.now()}-${Math.random().toString(16).slice(2)}`;const button=event.currentTarget.querySelector('button');button.disabled=true;button.textContent='Čuvam račun…';try{await storeReceiptImage(id,file);const receipt={id,store:String(form.get('store')).trim(),amount:Number(form.get('amount'))||0,date:String(form.get('date')),time:String(form.get('time')||'').trim(),receiptNumber:String(form.get('receiptNumber')||'').trim(),note:String(form.get('note')||'').trim(),createdAt:new Date().toISOString(),driveStatus:receiptDriveToken()?'sending':'local'};taskerReceipts.unshift(receipt);saveTaskerReceipts();if(receiptDriveToken()){button.textContent='Šaljem na Drive…';try{await uploadReceiptToDrive(receipt,file)}catch{receipt.driveStatus='error';saveTaskerReceipts()}}receiptProjectHome('overview')}catch{alert('Račun trenutno nije moguće sačuvati.')}})
