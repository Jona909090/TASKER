...
const defaultDashboardLayout={order:['tasks','modules','overview'],sizes:{tasks:6,modules:3,overview:3}}
const readDashboardLayout=()=>{try{const saved=JSON.parse(localStorage.getItem(dashboardLayoutStorage)||'{}'),valid=['tasks','modules','overview'];return{order:[...(saved.order||[]).filter((id)=>valid.includes(id)),...valid.filter((id)=>!(saved.order||[]).includes(id))],sizes:{...defaultDashboardLayout.sizes,...(saved.sizes||{})}}}catch{return JSON.parse(JSON.stringify(defaultDashboardLayout))}}
const saveDashboardLayout=(layout)=>localStorage.setItem(dashboardLayoutStorage,JSON.stringify(layout))
const applyDashboardLayout=()=>{const grid=document.querySelector('.dashboard-grid');if(!grid)return;const layout=readDashboardLayout();layout.order.forEach((id)=>{const card=grid.querySelector(`[data-dashboard-card="${id}"]`);if(card){card.style.setProperty('--card-span',layout.sizes[id]||4);grid.appendChild(card)}})}
const bindDashboardLayoutEditor=()=>{const grid=document.querySelector('.dashboard-grid'),toggle=document.querySelector('#dashboard-layout-toggle');if(!grid||!toggle)return;applyDashboardLayout();grid.querySelectorAll('[data-dashboard-card]').forEach((card)=>card.insertAdjacentHTML('afterbegin',`<div class="dashboard-card-tools"><button type="button" data-card-move="-1" title="Pomeri levo">←</button><button type="button" data-card-move="1" title="Pomeri desno">→</button><button type="button" data-card-size="-1" title="Smanji">−</button><button type="button" data-card-size="1" title="Povećaj">＋</button></div>`));toggle.addEventListener('click',()=>{grid.classList.toggle('layout-editing');toggle.classList.toggle('active');toggle.textContent=grid.classList.contains('layout-editing')?'✓ Završi raspored':'↔ Rasporedi kartice'});grid.addEventListener('click',(event)=>{const control=event.target.closest('[data-card-move],[data-card-size]');if(!control)return;event.preventDefault();event.stopPropagation();const card=control.closest('[data-dashboard-card]'),id=card.dataset.dashboardCard,layout=readDashboardLayout();if(control.dataset.cardMove){const index=layout.order.indexOf(id),target=index+Number(control.dataset.cardMove);if(target>=0&&target<layout.order.length)[layout.order[index],layout.order[target]]=[layout.order[target],layout.order[index]]}else{const sizes=[3,4,6,8,12],current=sizes.indexOf(Number(layout.sizes[id])||4),target=Math.max(0,Math.min(sizes.length-1,current+Number(control.dataset.cardSize)));layout.sizes[id]=sizes[target]}saveDashboardLayout(layout);applyDashboardLayout()})}
