...
const defaultModuleStages = ['Postavljanje folije', 'Postavljanje podnog lima (odozdo)', 'Postavljanje mineralne vune', 'Postavljanje plywooda', 'Postavljanje cetris ploca', 'Postavljanje okapne lajsne za panele', 'Postavljanje zidnih panela', 'Postavljanje stropnih panela', 'Silikoniranje vanjskog ruba panela i celika', 'Postavljanje kutne lajsne', 'Probijanje otvora', 'Postavljanje okapnih lajsni na modulu']
const newModuleStages = () => defaultModuleStages.map((title, index) => ({ id: `osnovno-${index + 1}`, title, status: 'nije-zavrseno', custom: false }))
...
function moduleUnitPage(typeId, unitId) {
  ...
  content.insertAdjacentHTML('beforeend', `<section class="plan-panel stages-panel"><header><div><h2>Faze rada modula</h2><p>Za svaku fazu oznacite trenutno stanje rada.</p></div><span>${doneStages}/${unit.stages.length} zavrseno</span></header><div id="module-stages" class="module-stages">${unit.stages.map((stage, index) => { const status = moduleStageStatus[stage.status] || moduleStageStatus['nije-zavrseno']; return `<article class="module-stage"><div class="stage-title"><span>${index + 1}</span><b>${esc(stage.title)}</b></div><div class="stage-status-actions"><button class="${stage.status === 'nije-zavrseno' ? 'selected ' : ''}stage-red" data-stage-status="nije-zavrseno" data-stage-id="${stage.id}" title="Nije zavrseno">&times;</button><button class="${stage.status === 'u-toku' ? 'selected ' : ''}stage-yellow" data-stage-status="u-toku" data-stage-id="${stage.id}" title="U toku">&#8226;</button><button class="${stage.status === 'zavrseno' ? 'selected ' : ''}stage-green" data-stage-status="zavrseno" data-stage-id="${stage.id}" title="Zavrseno">&#10003;</button>${stage.custom ? `<button class="stage-remove" data-remove-stage="${stage.id}" title="Obrisi fazu">&times;</button>` : ''}</div><small class="${status.className}">${status.label}</small></article>` }).join('')}</div><form id="module-stage-form" class="stage-add-form"><input name="stage-title" maxlength="120" placeholder="Dodajte novu fazu rada..." required><button class="primary-btn">+ Dodaj fazu</button></form></section>`)
  ...
  document.querySelector('#module-stage-form').addEventListener('submit', (event) => {
    event.preventDefault()
    const title = new FormData(event.currentTarget).get('stage-title').trim()
    if (!title) return
    unit.stages.push({ id: `dodatno-${Date.now()}`, title, status: 'nije-zavrseno', custom: true })
    saveModuleDetails()
