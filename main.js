...
function moduleUnit(type, number) {
  ...
  if (data.units[id].stageVersion !== 2) {
    ...
    const additions = [
      { id: 'osnovno-vuna-strop', title: 'Postavljanje mineralne vune u strop', status: 'nije-zavrseno', custom: false },
      { id: 'osnovno-folija-strop', title: 'Postavljanje folije u strop', status: 'nije-zavrseno', custom: false }
...
function moduleUnitPage(typeId, unitId) {
  ...
  document.querySelector('#module-stage-form').addEventListener('submit', (event) => {
    ...
    if (!title) return
    const newStage = { id: `dodatno-${Date.now()}`, title, status: 'nije-zavrseno', custom: true }
