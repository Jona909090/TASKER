...
async function receiptProjectHome(view='overview') {
  ...
  if(view==='drive') root.innerHTML=`<section class="receipt-drive-page"><span class="receipt-drive-icon">☁</span><p class="eyebrow">SINHRONIZACIJA U SLEDEĆEM KORAKU</p><h2>Povezivanje sa Google Drive-om</h2><p>Fotografije će se automatski raspoređivati u foldere po godini i prodavnici, a zbirna evidencija biće zajednička svim uređajima.</p><div><b>Predložena struktura</b><code>TASKER – Računi / 2026 / BAUHAUS<br>TASKER – Računi / 2026 / BAČELIĆ<br>TASKER – Računi / 2026 / OSTALE TRGOVINE</code></div><button type="button" disabled>Google Drive još nije povezan</button></section>`
  ...
  root.querySelector('#receipt-form')?.addEventListener('submit',async(event)=>{event.preventDefault();const form=new FormData(event.currentTarget),file=form.get('photo');if(!(file instanceof Blob)||!file.size)return;const id=`receipt-${Date.now()}-${Math.random().toString(16).slice(2)}`;const button=event.currentTarget.querySelector('button');button.disabled=true;button.textContent='Čuvam račun…';try{await storeReceiptImage(id,file);taskerReceipts.unshift({id,store:String(form.get('store')).trim(),amount:Number(form.get('amount'))||0,date:String(form.get('date')),time:String(form.get('time')||'').trim(),receiptNumber:String(form.get('receiptNumber')||'').trim(),note:String(form.get('note')||'').trim(),createdAt:new Date().toISOString()});saveTaskerReceipts();receiptProjectHome('overview')}catch{alert('Račun trenutno nije moguće sačuvati.')}})
