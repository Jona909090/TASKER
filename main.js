...
const storage = 'tasker.todos'; const filterStorage = 'tasker.filter'; const inventoryStorage = 'tasker.inventory'; const categoryStorage = 'tasker.categories'; const orderStorage = 'tasker.order-lines'; const moduleStorage = 'tasker.module-progress'; const moduleDetailStorage = 'tasker.module-details'; const employeeStorage = 'tasker.employees'; const attendanceStorage = 'tasker.daily-attendance'; const workHoursStorage = 'tasker.work-hours'; const workPlanStorage = 'tasker.work-plans'; const diaryStorage = 'tasker.work-diary'; const settingsStorage = 'tasker.settings'; const projectStorage = 'tasker.projects'; const receiptStorage = 'tasker.receipts'
const receiptDriveEndpoint = 'https://script.google.com/macros/s/AKfycbzF2SfEXGL137WzWsbhV6ElBEdqcVBbr5AFooCXWdtmuef-3pqaw2HCGnyUlFjoooTRHQ/exec'
const receiptDriveFolderUrl = 'https://drive.google.com/drive/folders/1wyGb-ZIEjmgaIlwseMlCRV33o0YGfsE1'
const receiptDriveTokenStorage = 'tasker.receipt-drive-token'
const receiptDriveTokenFromUrl = new URLSearchParams(location.search).get('driveToken')
if(receiptDriveTokenFromUrl){localStorage.setItem(receiptDriveTokenStorage,receiptDriveTokenFromUrl);const cleanUrl=new URL(location.href);cleanUrl.searchParams.delete('driveToken');history.replaceState({},'',cleanUrl)}
const receiptDriveToken = () => localStorage.getItem(receiptDriveTokenStorage)||''
...
let taskerReceipts = []
try { const saved = JSON.parse(localStorage.getItem(receiptStorage) || '[]'); taskerReceipts = Array.isArray(saved) ? saved : [] } catch {}
const saveTaskerReceipts = () => localStorage.setItem(receiptStorage, JSON.stringify(taskerReceipts))
function receiptImageDb() {
  return new Promise((resolve,reject)=>{
    const request=indexedDB.open('tasker-receipt-images',1)
...
async function storeReceiptImage(id,blob) {
  const db=await receiptImageDb()
  return new Promise((resolve,reject)=>{const transaction=db.transaction('images','readwrite');transaction.objectStore('images').put({id,blob});transaction.oncomplete=resolve;transaction.onerror=()=>reject(transaction.error)})
}
async function getReceiptImage(id) {
  const db=await receiptImageDb()
  return new Promise((resolve,reject)=>{const request=db.transaction('images','readonly').objectStore('images').get(id);request.onsuccess=()=>resolve(request.result?.blob||null);request.onerror=()=>reject(request.error)})
}
async function deleteReceiptImage(id) {
  const db=await receiptImageDb()
  return new Promise((resolve,reject)=>{const transaction=db.transaction('images','readwrite');transaction.objectStore('images').delete(id);transaction.oncomplete=resolve;transaction.onerror=()=>reject(transaction.error)})
}
const receiptMoney = (value) => new Intl.NumberFormat('hr-HR',{style:'currency',currency:'EUR'}).format(Number(value)||0)
const receiptDateLabel = (item) => { const date=String(item.date||'').split('-').reverse().join('.'); return `${date}${item.time ? ` · ${item.time}` : ''}` }
const receiptSearchText = (item) => [item.date,item.time,item.store,item.amount,item.receiptNumber,item.note].join(' ').toLocaleLowerCase('hr')
function receiptBlobBase64(blob){return new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(String(reader.result).split(',')[1]||'');reader.onerror=()=>reject(reader.error);reader.readAsDataURL(blob)})}
async function receiptDriveBlob(blob){try{const bitmap=await createImageBitmap(blob);const max=2200,scale=Math.min(1,max/Math.max(bitmap.width,bitmap.height));if(scale===1)return blob;const canvas=document.createElement('canvas');canvas.width=Math.round(bitmap.width*scale);canvas.height=Math.round(bitmap.height*scale);canvas.getContext('2d').drawImage(bitmap,0,0,canvas.width,canvas.height);bitmap.close?.();return await new Promise(resolve=>canvas.toBlob(value=>resolve(value||blob),'image/jpeg',.86))}catch{return blob}}
async function uploadReceiptToDrive(receipt,blob){const token=receiptDriveToken();if(!token)throw new Error('Drive nije podešen');const optimized=await receiptDriveBlob(blob),imageBase64=await receiptBlobBase64(optimized);await fetch(receiptDriveEndpoint,{method:'POST',mode:'no-cors',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({action:'uploadReceipt',token,imageBase64,mimeType:optimized.type||blob.type||'image/jpeg',store:receipt.store,amount:receipt.amount,date:receipt.date,time:receipt.time,receiptNumber:receipt.receiptNumber,note:receipt.note})});receipt.driveStatus='sent';receipt.driveSentAt=new Date().toISOString();saveTaskerReceipts();return true}
let receiptDriveAutoSync=false
const receiptIdentity=(item)=>[String(item.store||'').toLocaleLowerCase('hr'),Number(item.amount||0).toFixed(2),item.date||'',item.time||'',String(item.receiptNumber||'').toLocaleLowerCase('hr')].join('|')
async function syncReceiptsFromDrive(status){
 const token=receiptDriveToken();if(!token)throw new Error('Drive nije podešen');if(status)status.textContent='Preuzimam račune sa Drive-a…'
 const response=await fetch(`${receiptDriveEndpoint}?action=listReceipts&token=${encodeURIComponent(token)}&_=${Date.now()}`,{cache:'no-store'})
 const data=await response.json();if(!data.success||!Array.isArray(data.receipts))throw new Error(data.error||'Drive evidencija nije dostupna')
 const known=new Map(taskerReceipts.map(item=>[receiptIdentity(item),item]));let added=0
 data.receipts.forEach(remote=>{const normalized={id:`drive-${remote.fileId}`,store:remote.store||remote.folder||'Ostalo',amount:Number(remote.amount)||0,date:remote.date||'',time:remote.time||'',receiptNumber:remote.receiptNumber||'',note:remote.note||'',createdAt:remote.createdAt||remote.modifiedAt||new Date().toISOString(),driveStatus:'sent',driveFileId:remote.fileId,driveUrl:remote.fileUrl,driveOnly:true};const key=receiptIdentity(normalized),local=known.get(key);if(local){local.driveStatus='sent';local.driveFileId=remote.fileId;local.driveUrl=remote.fileUrl}else{taskerReceipts.push(normalized);known.set(key,normalized);added++}})
 saveTaskerReceipts();if(status){status.textContent=`✓ Drive evidencija je osvežena · ${data.receipts.length} računa`;status.classList.add('ok')}return {added,total:data.receipts.length}
}
async function syncPendingReceipts(status){const pending=taskerReceipts.filter(item=>item.driveStatus!=='sent');let sent=0;for(const receipt of pending){const blob=await getReceiptImage(receipt.id);if(!blob)continue;if(status)status.textContent=`Šaljem ${sent+1} od ${pending.length}…`;try{await uploadReceiptToDrive(receipt,blob);sent++}catch{receipt.driveStatus='error';saveTaskerReceipts()}}return {sent,total:pending.length}}
const receiptOcrScript='https://cdn.jsdelivr.net/npm/tesseract.js@6.0.1/dist/tesseract.min.js'
let receiptOcrLoading=null
function loadReceiptOcr(){if(window.Tesseract)return Promise.resolve(window.Tesseract);if(receiptOcrLoading)return receiptOcrLoading;receiptOcrLoading=new Promise((resolve,reject)=>{const script=document.createElement('script');script.src=receiptOcrScript;script.async=true;script.onload=()=>resolve(window.Tesseract);script.onerror=()=>reject(new Error('OCR biblioteka nije učitana'));document.head.appendChild(script)});return receiptOcrLoading}
const receiptKnownStores=[['BAUHAUS',/bauhaus/i],['BAČELIĆ',/ba[čc]eli[ćc]/i],['PEVEX',/pevex|pevec/i],['HORNBACH',/hornbach/i],['WÜRTH',/w[uü]rth/i],['HILTI',/hilti/i],['FEROTERM',/feroterm/i],['SAMOBORKA',/samoborka/i],['CIOS',/cios/i]]
function receiptIsoDate(day,month,year){const y=String(year).length===2?`20${year}`:String(year);const d=String(day).padStart(2,'0'),m=String(month).padStart(2,'0');if(+m<1||+m>12||+d<1||+d>31)return '';return `${y}-${m}-${d}`}
function parseReceiptOcr(text){
 const clean=String(text||'').replace(/\r/g,'\n').replace(/[|]/g,'I'),lines=clean.split(/\n+/).map(line=>line.replace(/\s+/g,' ').trim()).filter(Boolean)
 const joined=lines.join('\n'),known=receiptKnownStores.find(([,pattern])=>pattern.test(joined))
...
async function scanReceiptFile(file,formRoot){
 const status=formRoot.querySelector('#receipt-ocr-status'),button=formRoot.querySelector('#receipt-ocr-button'),form=formRoot.querySelector('#receipt-form');if(!file||!status||!button)return
 button.disabled=true;status.hidden=false;status.className='receipt-ocr-status working';status.innerHTML='<b>Pripremam čitanje računa…</b><span><i style="width:5%"></i></span><small>Prvo očitavanje može trajati malo duže.</small>'
 let worker
 try{const Tesseract=await loadReceiptOcr();worker=await Tesseract.createWorker(['hrv','eng'],1,{logger:(message)=>{if(message.status==='recognizing text'){const percent=Math.max(8,Math.round((message.progress||0)*100));status.querySelector('b').textContent=`Očitavam račun… ${percent}%`;status.querySelector('i').style.width=`${percent}%`}}});const result=await worker.recognize(file);const parsed=parseReceiptOcr(result.data.text);for(const [name,value] of Object.entries(parsed)){if(['text','confidence'].includes(name)||value===''||value===0)continue;const field=form.elements[name];if(field)field.value=value}status.className='receipt-ocr-status done';status.innerHTML=`<b>✓ Račun je očitan — proverite podatke</b><p><span>${parsed.store?'Prodavnica pronađena':'Proverite prodavnicu'}</span><span>${parsed.amount?'Iznos pronađen':'Proverite iznos'}</span><span>${parsed.date?'Datum pronađen':'Proverite datum'}</span><span>${parsed.time?'Vreme pronađeno':'Proverite vreme'}</span></p><details><summary>Prikaži očitani tekst</summary><pre>${esc(parsed.text)}</pre></details>`;form.classList.add('receipt-ocr-reviewed')}catch(error){console.error(error);status.className='receipt-ocr-status error';status.innerHTML='<b>Očitavanje nije uspelo</b><small>Proverite internet, napravite jasniju fotografiju i pokušajte ponovo. Podatke i dalje možete uneti ručno.</small>'}finally{if(worker)await worker.terminate().catch(()=>{});button.disabled=false;button.textContent='✦ Ponovo očitaj račun'}
}
const receiptStoreKey = (value) => String(value||'Ostalo').trim().toLocaleUpperCase('hr')
const receiptStores = () => {
  const map=new Map()
  taskerReceipts.forEach((receipt)=>{const key=receiptStoreKey(receipt.store);if(!map.has(key))map.set(key,{name:receipt.store||'Ostalo',count:0,total:0});const item=map.get(key);item.count+=1;item.total+=Number(receipt.amount)||0})
...
function receiptProjectShell(active='overview') {
  ...
  return `<section class="receipt-project"><header class="receipt-project-header"><button type="button" data-receipt-back>← Projekti</button><div><p class="eyebrow">TASKER · PROJEKAT 02</p><h1>Računi i troškovi</h1><p>Svi plaćeni računi i ukupni troškovi na jednom mestu.</p></div><span class="receipt-project-mark">€</span></header><nav class="receipt-project-nav">${tabs.map(([id,icon,label])=>`<button type="button" data-receipt-view="${id}" class="${active===id?'active':''}"><span>${icon}</span>${label}</button>`).join('')}</nav><div id="receipt-project-content"></div></section>`
}
function receiptSummary() {
  const today=dateKeyFor(),month=today.slice(0,7)
  const total=taskerReceipts.reduce((sum,item)=>sum+(Number(item.amount)||0),0)
  const todayTotal=taskerReceipts.filter((item)=>item.date===today).reduce((sum,item)=>sum+(Number(item.amount)||0),0)
  const monthTotal=taskerReceipts.filter((item)=>String(item.date).startsWith(month)).reduce((sum,item)=>sum+(Number(item.amount)||0),0)
  return {total,todayTotal,monthTotal}
}
function receiptRows(receipts=taskerReceipts) {
  return receipts.length ? receipts.slice().sort((a,b)=>`${b.date||''} ${b.time||''}`.localeCompare(`${a.date||''} ${a.time||''}`)||String(b.createdAt).localeCompare(String(a.createdAt))).map((item)=>`<article class="receipt-table-row"><span><b>${String(item.date||'').split('-').reverse().join('.')}</b><small>${esc(item.time||'Vreme nije uneto')}</small></span><div><b>${esc(item.store)}</b><small>${esc(item.receiptNumber||'Bez broja računa')}${item.driveStatus==='sent'?' · ☁ Drive':''}</small></div><strong>${receiptMoney(item.amount)}</strong><span>${esc(item.note||'—')}</span><div><button type="button" data-receipt-image="${item.id}">▧ Račun</button><button type="button" class="delete" data-receipt-delete="${item.id}">×</button></div></article>`).join('') : '<p class="receipt-empty">Još nema sačuvanih računa.</p>'
}
async function receiptProjectHome(view='overview') {
  projectOpen=false
  document.querySelector('.shell').classList.add('project-home','receipt-project-open')
  content.innerHTML=receiptProjectShell(view)
  const root=document.querySelector('#receipt-project-content')
  const summary=receiptSummary(),stores=receiptStores()
  if(view==='overview') root.innerHTML=`<section class="receipt-kpis"><article class="green"><span>€</span><p>Danas</p><strong>${receiptMoney(summary.todayTotal)}</strong><small>${dateKeyFor().split('-').reverse().join('.')}</small></article><article class="blue"><span>▦</span><p>Ovaj mesec</p><strong>${receiptMoney(summary.monthTotal)}</strong><small>mesečni zbir</small></article><article class="gold"><span>∑</span><p>Ukupno</p><strong>${receiptMoney(summary.total)}</strong><small>${taskerReceipts.length} računa</small></article><article class="pink"><span>⌂</span><p>Prodavnice</p><strong>${stores.length}</strong><small>automatske grupe</small></article></section><section class="receipt-overview-grid"><article class="receipt-panel"><header><div><h2>Poslednji računi</h2><p>Najnovije sačuvane kupovine.</p></div><button data-receipt-view="table">Prikaži sve →</button></header><div class="receipt-mini-list">${taskerReceipts.slice().sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt))).slice(0,6).map((item)=>`<div><span>▧</span><div><b>${esc(item.store)}</b><small>${receiptDateLabel(item)}</small></div><strong>${receiptMoney(item.amount)}</strong></div>`).join('')||'<p class="receipt-empty">Dodajte prvi račun.</p>'}</div></article><article class="receipt-panel"><header><div><h2>Trošak po prodavnici</h2><p>Zbir svih računa po trgovini.</p></div></header><div class="receipt-store-bars">${stores.slice(0,6).map((store,index)=>`<div><span>${esc(store.name)}</span><i><em style="width:${summary.total?Math.max(5,store.total/summary.total*100):0}%;--bar:${['#51ff83','#3bd7ff','#ffd33d','#ff58c8','#8e7dff'][index%5]}"></em></i><b>${receiptMoney(store.total)}</b></div>`).join('')||'<p class="receipt-empty">Nema podataka.</p>'}</div></article></section><section class="receipt-drive-banner"><span>☁</span><div><b>Google Drive sinhronizacija</b><p>Sledeći korak: slike računa biće dostupne na telefonu, tabletu i računaru.</p></div><em>ČEKA POVEZIVANJE</em></section>`
  if(view==='new') root.innerHTML=`<section class="receipt-entry-layout"><form id="receipt-form" class="receipt-form"><header><p class="eyebrow">NOVI TROŠAK</p><h2>Fotografiši i sačuvaj račun</h2><p>TASKER automatski čita račun. Vi samo proverite podatke pre čuvanja.</p></header><label class="receipt-photo-field"><span>▧</span><b>Fotografiši ili izaberi račun</b><small>JPG, PNG ili fotografija kamere</small><input id="receipt-photo" name="photo" type="file" accept="image/*" capture="environment" required></label><div id="receipt-photo-preview" class="receipt-photo-preview" hidden><img alt="Pregled računa"><button type="button" id="receipt-ocr-button">✦ Očitaj i automatski popuni</button></div><div id="receipt-ocr-status" class="receipt-ocr-status" hidden></div><div class="receipt-form-grid"><label>Prodavnica<input name="store" list="receipt-store-options" required placeholder="npr. BAUHAUS"><datalist id="receipt-store-options"><option value="BAUHAUS"><option value="BAČELIĆ"></datalist></label><label>Ukupan iznos (€)<input name="amount" type="number" min="0.01" step="0.01" required placeholder="20,32"></label><label>Datum izdavanja računa<input name="date" type="date" required value="${dateKeyFor()}"></label><label>Vreme izdavanja<input name="time" type="time" required value="${new Date().toTimeString().slice(0,5)}"></label><label>Broj računa<input name="receiptNumber" placeholder="nije obavezno"></label><label class="wide">Napomena<input name="note" placeholder="materijal ili razlog kupovine"></label></div><div class="receipt-form-actions"><span>Fotografija se očitava na ovom uređaju. Pre čuvanja proverite iznos i datum.</span><button class="primary-btn">Sačuvaj račun</button></div></form></section>`
  if(view==='stores') root.innerHTML=`<section class="receipt-section-heading"><div><p class="eyebrow">AUTOMATSKE GRUPE</p><h2>Prodavnice</h2><p>Nova prodavnica dobija svoju karticu čim sačuvate prvi račun.</p></div><b>${stores.length} prodavnica</b></section><section class="receipt-store-grid">${stores.map((store,index)=>`<article style="--store-color:${['#51ff83','#3bd7ff','#ffd33d','#ff58c8','#8e7dff'][index%5]}"><span>${esc(store.name.trim().charAt(0).toUpperCase())}</span><div><p>PRODAVNICA</p><h2>${esc(store.name)}</h2><small>${store.count} računa</small></div><strong>${receiptMoney(store.total)}</strong></article>`).join('')||'<p class="receipt-empty">Prodavnice će se pojaviti nakon prvog računa.</p>'}</section>`
  if(view==='table') root.innerHTML=`<section class="receipt-section-heading"><div><p class="eyebrow">ZBIRNA EVIDENCIJA</p><h2>Svi računi</h2><p>Datum, vreme, prodavnica, iznos i fotografija računa.</p></div><strong>${receiptMoney(summary.total)}</strong></section><section class="receipt-filter"><span>⌕</span><input id="receipt-search" type="search" placeholder="Pretraži datum, vreme, prodavnicu, iznos ili broj računa…"><input id="receipt-date-filter" type="date" aria-label="Filtriraj po datumu"><button type="button" id="receipt-clear-filter">Obriši filter</button></section><section class="receipt-table"><header><span>Datum</span><span>Prodavnica</span><span>Iznos</span><span>Napomena</span><span>Dokument</span></header><div>${receiptRows()}</div><footer><span>UKUPNO</span><b>${receiptMoney(summary.total)}</b></footer></section>`
  if(view==='drive') root.innerHTML=`<section class="receipt-drive-page receipt-drive-connected"><span class="receipt-drive-icon">☁</span><p class="eyebrow">GOOGLE DRIVE SINHRONIZACIJA</p><h2>${receiptDriveToken()?'Drive je povezan':'Završite povezivanje uređaja'}</h2><p>Fotografije računa automatski se šalju u TASKER foldere po godini i prodavnici. Tajni kod ostaje sačuvan samo na ovom uređaju.</p><div class="receipt-drive-structure"><b>Aktivni folder</b><code>TASKER – Računi / godina / prodavnica</code></div><label class="receipt-drive-token">Tajni kod uređaja<input id="receipt-drive-token" type="password" value="${esc(receiptDriveToken())}" placeholder="Nalepite privatni kod"><small>Unosi se jednom na svakom telefonu, tabletu ili računaru.</small></label><p id="receipt-drive-status" class="receipt-drive-status">${receiptDriveToken()?'● Veza je podešena na ovom uređaju':'○ Unesite tajni kod i sačuvajte vezu'}</p><div class="receipt-drive-actions"><button type="button" id="receipt-drive-save">Sačuvaj vezu</button><button type="button" id="receipt-drive-test">Proveri vezu</button><button type="button" id="receipt-drive-download">Preuzmi sa Drive-a</button><button type="button" id="receipt-drive-sync">Pošalji ranije račune</button><a href="${receiptDriveFolderUrl}" target="_blank" rel="noopener">Otvori Drive folder ↗</a></div></section>`
  if(receiptDriveToken()&&!receiptDriveAutoSync&&view!=='new'){receiptDriveAutoSync=true;syncReceiptsFromDrive().then(result=>{if(result.added)receiptProjectHome(view)}).catch(error=>console.warn('Drive sync:',error.message))}
  root.querySelector('#receipt-photo')?.addEventListener('change',(event)=>{const file=event.target.files?.[0],preview=root.querySelector('#receipt-photo-preview');if(!file||!preview)return;preview.hidden=false;preview.querySelector('img').src=URL.createObjectURL(file);root.querySelector('#receipt-ocr-button').textContent='✦ Očitaj i automatski popuni';scanReceiptFile(file,root)})
  root.querySelector('#receipt-ocr-button')?.addEventListener('click',()=>scanReceiptFile(root.querySelector('#receipt-photo')?.files?.[0],root))
  root.querySelector('#receipt-form')?.addEventListener('submit',async(event)=>{event.preventDefault();const form=new FormData(event.currentTarget),file=form.get('photo');if(!(file instanceof Blob)||!file.size)return;const id=`receipt-${Date.now()}-${Math.random().toString(16).slice(2)}`;const button=event.currentTarget.querySelector('.receipt-form-actions button');button.disabled=true;button.textContent='Čuvam račun…';try{await storeReceiptImage(id,file);const receipt={id,store:String(form.get('store')).trim(),amount:Number(form.get('amount'))||0,date:String(form.get('date')),time:String(form.get('time')||'').trim(),receiptNumber:String(form.get('receiptNumber')||'').trim(),note:String(form.get('note')||'').trim(),createdAt:new Date().toISOString(),driveStatus:receiptDriveToken()?'sending':'local'};taskerReceipts.unshift(receipt);saveTaskerReceipts();if(receiptDriveToken()){button.textContent='Šaljem na Drive…';try{await uploadReceiptToDrive(receipt,file)}catch{receipt.driveStatus='error';saveTaskerReceipts()}}receiptProjectHome('overview')}catch{alert('Račun trenutno nije moguće sačuvati.')}})
  const driveStatus=root.querySelector('#receipt-drive-status')
  root.querySelector('#receipt-drive-save')?.addEventListener('click',()=>{const token=root.querySelector('#receipt-drive-token').value.trim();if(!token){localStorage.removeItem(receiptDriveTokenStorage);driveStatus.textContent='○ Tajni kod nije unet';return}localStorage.setItem(receiptDriveTokenStorage,token);driveStatus.textContent='● Veza je sačuvana na ovom uređaju';driveStatus.classList.add('ok')})
  root.querySelector('#receipt-drive-test')?.addEventListener('click',async()=>{const token=root.querySelector('#receipt-drive-token').value.trim();if(!token){driveStatus.textContent='Unesite tajni kod.';return}driveStatus.textContent='Proveravam vezu…';try{await fetch(`${receiptDriveEndpoint}?token=${encodeURIComponent(token)}`,{mode:'no-cors',cache:'no-store'});localStorage.setItem(receiptDriveTokenStorage,token);driveStatus.textContent='✓ TASKER je povezan sa Google Drive-om';driveStatus.classList.add('ok')}catch{driveStatus.textContent='Veza nije dostupna. Proverite internet i tajni kod.';driveStatus.classList.remove('ok')}})
  root.querySelector('#receipt-drive-download')?.addEventListener('click',async(event)=>{if(!receiptDriveToken()){driveStatus.textContent='Prvo sačuvajte tajni kod.';return}event.currentTarget.disabled=true;try{await syncReceiptsFromDrive(driveStatus);setTimeout(()=>receiptProjectHome('table'),500)}catch(error){driveStatus.textContent='Preuzimanje nije uspelo: '+error.message;driveStatus.classList.remove('ok')}finally{event.currentTarget.disabled=false}})
  root.querySelector('#receipt-drive-sync')?.addEventListener('click',async(event)=>{if(!receiptDriveToken()){driveStatus.textContent='Prvo sačuvajte tajni kod.';return}event.currentTarget.disabled=true;const result=await syncPendingReceipts(driveStatus);driveStatus.textContent=`✓ Poslato ${result.sent} od ${result.total} ranijih računa`;driveStatus.classList.add('ok');event.currentTarget.disabled=false})
  const receiptSearch=root.querySelector('#receipt-search'),receiptDateFilter=root.querySelector('#receipt-date-filter'),receiptTableBody=root.querySelector('.receipt-table>div');const filterReceipts=()=>{if(!receiptTableBody)return;const query=String(receiptSearch?.value||'').trim().toLocaleLowerCase('hr'),date=String(receiptDateFilter?.value||'');const filtered=taskerReceipts.filter((item)=>(!query||receiptSearchText(item).includes(query))&&(!date||item.date===date));receiptTableBody.innerHTML=receiptRows(filtered)};receiptSearch?.addEventListener('input',filterReceipts);receiptDateFilter?.addEventListener('change',filterReceipts);root.querySelector('#receipt-clear-filter')?.addEventListener('click',()=>{receiptSearch.value='';receiptDateFilter.value='';filterReceipts()})
  content.querySelectorAll('[data-receipt-view]').forEach((button)=>button.addEventListener('click',()=>receiptProjectHome(button.dataset.receiptView)))
  content.querySelector('[data-receipt-back]')?.addEventListener('click',projectsHome)
  root.addEventListener('click',async(event)=>{const imageButton=event.target.closest('[data-receipt-image]');if(imageButton){const receipt=taskerReceipts.find(item=>item.id===imageButton.dataset.receiptImage),blob=await getReceiptImage(imageButton.dataset.receiptImage);if(!blob){if(receipt?.driveUrl)window.open(receipt.driveUrl,'_blank','noopener');return}const url=URL.createObjectURL(blob);const modal=document.createElement('div');modal.className='receipt-image-modal';modal.innerHTML=`<section><button type="button">×</button><img src="${url}" alt="Fotografija računa"></section>`;const close=()=>{URL.revokeObjectURL(url);modal.remove()};modal.addEventListener('click',(event)=>{if(event.target===modal||event.target.closest('button'))close()});document.body.appendChild(modal);return}const deleteButton=event.target.closest('[data-receipt-delete]');if(deleteButton){if(!confirm('Obrisati ovaj račun?'))return;await deleteReceiptImage(deleteButton.dataset.receiptDelete);taskerReceipts=taskerReceipts.filter((item)=>item.id!==deleteButton.dataset.receiptDelete);saveTaskerReceipts();receiptProjectHome('table')}})
}
...
function employeeHubTabs(active = 'employees') {
  const tabs = [
    { id: 'employees', icon: '☻', label: 'Podaci radnika', text: 'Imena, pozicije i telefoni' },
    { id: 'daily-report', icon: '✓', label: 'Dnevna prisutnost', text: 'Aktivni i odsutni radnici' },
    { id: 'work-hours', icon: '◷', label: 'Radni sati', text: 'Dnevni unos sati' },
    { id: 'monthly-hours', icon: '▦', label: 'Mesečni sati', text: 'Mesečni zbir i obračun' }
  ]
  return `<section class="employee-hub"><header><div><p class="eyebrow">LJUDI I EVIDENCIJA RADA</p><h1>Zaposleni</h1><p>Svi podaci o radnicima, prisutnosti i satima objedinjeni su na jednom mestu.</p></div><span>${state.employees.length} radnika</span></header><nav>${tabs.map((tab) => `<button type="button" data-employee-hub="${tab.id}" class="${active === tab.id ? 'active' : ''}"><span>${tab.icon}</span><div><b>${tab.label}</b><small>${tab.text}</small></div></button>`).join('')}</nav></section>`
}

content.addEventListener('click', (event) => {
  const tab = event.target.closest('[data-employee-hub]')?.dataset.employeeHub
  if (!tab) return
  if (tab === 'employees') employeesPage()
  else if (tab === 'daily-report') dailyReportPage()
  else if (tab === 'work-hours') workHoursPage()
  else if (tab === 'monthly-hours') monthlyHoursPage()
})

function employeesPage() {
  content.innerHTML = employeeHubTabs('employees') + `<section class="page-heading employee-heading compact-employee-heading"><div><p class="eyebrow">Tim i organizacija</p><h1>Zaposleni</h1><p>Pregled radnika, pozicija i telefona.</p></div><div class="employee-heading-actions"><span class="employee-count">${state.employees.length} zaposlenih</span><button class="primary-btn" id="add-employee">+ Dodaj zaposlenog</button></div></section><section class="employee-tools"><div class="employee-search"><span>\u2315</span><input id="employee-search" placeholder="Pretrazi zaposlenog..."></div><span>Svi zaposleni</span></section><section class="employee-grid">${state.employees.map((employee) => { const initials = employee.name.split(' ').map((part) => part[0]).slice(0, 2).join('').toUpperCase(); return `<article class="employee-card employee-directory-card" data-employee="${employee.name.toLocaleLowerCase('sr')}"><header><span class="employee-avatar">${initials}</span><div><h2>${esc(employee.name)}</h2><p>${esc(employee.role)}</p></div></header><div class="employee-details"><p><span>\u25C9</span> Pozicija: <strong>${esc(employee.role)}</strong></p><p><span>\u260E</span> Telefon: <strong>${esc(employee.phone)}</strong></p></div><div class="employee-card-actions directory-actions"><button class="employee-delete" data-delete-employee="${employee.id}" title="Obriši zaposlenog">× Obriši</button></div></article>` }).join('')}</section>`
...
function attendanceFor(dateKey) {
  if (!state.attendance[dateKey]) {
    state.attendance[dateKey] = Object.fromEntries(state.employees.map((employee) => [employee.id, true]))
    saveAttendance()
  }
...
function workforcePdfDb() {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open('tasker-workforce-pdfs', 1)
    request.onupgradeneeded = () => {
      if (!request.result.objectStoreNames.contains('pdfs')) request.result.createObjectStore('pdfs', { keyPath: 'id' })
    }
    request.onsuccess = () => resolve(request.result)
...
async function storeWorkforcePdf(entry) {
  const db = await workforcePdfDb()
  return new Promise((resolve, reject) => {
    const transaction = db.transaction('pdfs', 'readwrite')
    transaction.objectStore('pdfs').put(entry)
    transaction.oncomplete = resolve
...
async function listWorkforcePdfs(type) {
  const db = await workforcePdfDb()
  return new Promise((resolve, reject) => {
    const request = db.transaction('pdfs', 'readonly').objectStore('pdfs').getAll()
    request.onsuccess = () => resolve(request.result.filter((entry) => !type || entry.type === type).sort((a,b) => String(b.createdAt).localeCompare(String(a.createdAt))))
    request.onerror = () => reject(request.error)
  })
}
async function deleteWorkforcePdf(id) {
  const db = await workforcePdfDb()
  return new Promise((resolve, reject) => {
    const transaction = db.transaction('pdfs', 'readwrite')
    transaction.objectStore('pdfs').delete(id)
...
function workforceArchiveSection(type, title) {
  return `<section class="workforce-pdf-archive"><header><div><span>PDF</span><div><h2>${esc(title)}</h2><p>Svako čuvanje ostaje kao poseban dokument.</p></div></div></header><div class="workforce-pdf-list" data-workforce-list="${type}"><p class="control-empty">Učitavam dokumente…</p></div></section>`
}
async function renderWorkforceArchive(type) {
  const list = document.querySelector(`[data-workforce-list="${type}"]`)
  if (!list) return
  try {
    const entries = await listWorkforcePdfs(type)
    list.innerHTML = entries.length ? entries.map((entry) => `<article><span>PDF</span><div><b>${esc(entry.fileName)}</b><small>${esc(entry.periodLabel)} · ${new Intl.DateTimeFormat('sr-Latn-RS',{dateStyle:'medium',timeStyle:'short'}).format(new Date(entry.createdAt))}</small></div><div><button type="button" data-workforce-share="${entry.id}">↗ Pošalji</button><button type="button" data-workforce-open="${entry.id}">Otvori</button><button type="button" class="delete" data-workforce-delete="${entry.id}">×</button></div></article>`).join('') : '<p class="control-empty">Još nema sačuvanih PDF dokumenata.</p>'
  } catch {
    list.innerHTML = '<p class="control-empty">PDF arhiva trenutno nije dostupna.</p>'
  }
}
async function createWorkforcePdf({ type, title, periodLabel, filePrefix, columns, rows }) {
  const [jsPDF, html2canvas] = await Promise.all([loadPdfLibrary(), loadHtml2Canvas()])
  const sheet = document.createElement('section')
  sheet.className = 'workforce-pdf-sheet'
  sheet.style.cssText = 'position:fixed;left:-10000px;top:0;width:794px;height:1123px;overflow:hidden;box-sizing:border-box;background:#0b1424;color:#edf7ff;font-family:Arial,sans-serif;padding:42px 48px;'
  sheet.innerHTML = `<header><div class="workforce-pdf-logo"><i></i><b>T</b><i></i></div><div><p>TASKER · EVIDENCIJA ZAPOSLENIH</p><h1>${esc(title)}</h1></div><aside><span>${esc(periodLabel)}</span><small>Pripremio: ${esc(state.settings.userName || 'Stefan Jonić')}</small></aside></header><main><div class="workforce-pdf-head">${columns.map((column)=>`<span>${esc(column)}</span>`).join('')}</div><div class="workforce-pdf-rows">${rows.map((row,index)=>`<div><span>${index+1}</span>${row.map((cell)=>`<span>${esc(cell)}</span>`).join('')}</div>`).join('')}</div></main><footer><span>TASKER · Sve pod kontrolom.</span><span>${new Intl.DateTimeFormat('sr-Latn-RS',{dateStyle:'long',timeStyle:'short'}).format(new Date())}</span></footer><style>
  .workforce-pdf-sheet *{box-sizing:border-box}.workforce-pdf-sheet header{display:grid;grid-template-columns:70px 1fr auto;align-items:center;gap:18px;padding-bottom:28px;border-bottom:2px solid #2a4968}.workforce-pdf-logo{display:grid;justify-items:center;gap:3px}.workforce-pdf-logo i{width:30px;height:7px;border-radius:8px;background:#f0565e}.workforce-pdf-logo i:last-child{background:#eef5fa}.workforce-pdf-logo b{display:grid;place-items:center;width:48px;height:48px;border-radius:14px;background:#3883f2;font-size:27px}.workforce-pdf-sheet header p{margin:0;color:#55d7ff;font-size:10px;font-weight:800;letter-spacing:.13em}.workforce-pdf-sheet h1{margin:7px 0 0;font-size:29px}.workforce-pdf-sheet aside{display:grid;gap:6px;text-align:right}.workforce-pdf-sheet aside span{color:#5dff91;font-weight:800}.workforce-pdf-sheet aside small{color:#91a7bc}.workforce-pdf-sheet main{margin-top:28px;border:1px solid #294763;border-radius:12px;overflow:hidden}.workforce-pdf-head,.workforce-pdf-rows>div{display:grid;grid-template-columns:45px repeat(${columns.length},1fr);align-items:center}.workforce-pdf-head{padding:13px;background:#1a4163;color:#dff5ff;font-size:11px;font-weight:800}.workforce-pdf-head:before{content:"R.br."}.workforce-pdf-rows>div{min-height:49px;padding:8px 13px;border-top:1px solid #29435f;background:#11223a}.workforce-pdf-rows>div:nth-child(even){background:#152943}.workforce-pdf-rows span{padding-right:8px;font-size:11px}.workforce-pdf-sheet footer{position:absolute;left:48px;right:48px;bottom:36px;display:flex;justify-content:space-between;padding-top:12px;border-top:1px solid #294763;color:#8299af;font-size:10px}
</style>`
  document.body.appendChild(sheet)
  try {
    const canvas = await html2canvas(sheet,{scale:2,backgroundColor:'#0b1424',useCORS:true})
    const pdf = new jsPDF({unit:'mm',format:'a4'})
    pdf.addImage(canvas.toDataURL('image/jpeg',.94),'JPEG',0,0,210,297)
    const createdAt = new Date().toISOString()
    const stamp = createdAt.replace(/[-:]/g,'').slice(0,15)
    const entry = { id: `${type}-${Date.now()}-${Math.random().toString(16).slice(2)}`, type, periodLabel, createdAt, fileName: `${filePrefix}-${stamp}.pdf`, blob: pdf.output('blob') }
    await storeWorkforcePdf(entry)
    return entry
  } finally {
    sheet.remove()
  }
...
content.addEventListener('click', async (event) => {
  const openId = event.target.closest('[data-workforce-open]')?.dataset.workforceOpen
  const shareId = event.target.closest('[data-workforce-share]')?.dataset.workforceShare
  const deleteId = event.target.closest('[data-workforce-delete]')?.dataset.workforceDelete
  if (!openId && !shareId && !deleteId) return
  const entries = await listWorkforcePdfs()
  const entry = entries.find((item) => item.id === (openId || shareId || deleteId))
  if (!entry) return
  if (openId) openTaskerPdfViewer(entry, 'TASKER · Evidencija zaposlenih')
  if (shareId) await shareTaskerPdf(entry, 'TASKER · Evidencija zaposlenih', entry.periodLabel)
  if (deleteId && confirm('Obrisati ovaj PDF dokument?')) {
    await deleteWorkforcePdf(entry.id)
    renderWorkforceArchive(entry.type)
  }
})
...
function dailyReportPage(dateKey = dateKeyFor()) {
  ...
  content.innerHTML = employeeHubTabs('daily-report') + `<section class="page-heading attendance-heading compact-employee-heading"><div><p class="eyebrow">Kontrolna tabla rada</p><h1>Dnevno stanje ljudi</h1><p>Izaberite datum i označite ko je bio aktivan na gradilištu.</p></div><label class="attendance-date"><span>Datum</span><input id="attendance-date" type="date" value="${dateKey}"></label></section><section class="attendance-stats"><article><span class="stat-icon blue">☷</span><p>Ukupno zaposlenih</p><strong>${state.employees.length}</strong><small>evidencija za izabrani dan</small></article><article><span class="stat-icon green">✓</span><p>Prisutni danas</p><strong>${activeToday.length}</strong><small>prisutni na radu</small></article><article><span class="stat-icon red">×</span><p>Odsutni danas</p><strong>${inactiveToday.length}</strong><small>nisu bili na radu</small></article></section><section class="attendance-panel"><header><div><h2>Prisustvo zaposlenih</h2><p>${dateTitle.charAt(0).toLocaleUpperCase('sr')}${dateTitle.slice(1)}</p></div><span>Promene se čuvaju automatski</span></header><div class="attendance-list">${state.employees.length ? state.employees.map((employee) => { const initials = employee.name.split(' ').map((part) => part[0]).slice(0, 2).join('').toUpperCase(); const active = attendance[employee.id] !== false; return `<article class="attendance-row"><span class="employee-avatar">${initials}</span><div class="attendance-person"><b>${esc(employee.name)}</b><small>${esc(employee.role)}</small></div><div class="attendance-actions"><button class="attendance-status ${active ? 'selected-active' : ''}" data-attendance="active" data-employee-id="${employee.id}">Prisutan</button><button class="attendance-status ${!active ? 'selected-inactive' : ''}" data-attendance="inactive" data-employee-id="${employee.id}">Odsutan</button></div></article>` }).join('') : '<p class="attendance-empty">Nema zaposlenih za evidenciju.</p>'}</div></section><section class="workforce-pdf-create"><div><h2>PDF dnevne prisutnosti</h2><p>Sačuvajte trenutno označene prisutne i odsutne radnike za izabrani datum.</p></div><button type="button" class="primary-btn" id="save-attendance-pdf">Sačuvaj PDF</button></section>${workforceArchiveSection('attendance','Sačuvani PDF izveštaji prisutnosti')}`

  renderWorkforceArchive('attendance')
  document.querySelector('#save-attendance-pdf').addEventListener('click', async (event) => {
    const button = event.currentTarget
    button.disabled = true
    button.textContent = 'Pravim PDF…'
    try {
      await createWorkforcePdf({ type:'attendance', title:'DNEVNA PRISUTNOST', periodLabel:dateKey.split('-').reverse().join('.'), filePrefix:`Prisutnost-${dateKey}`, columns:['Ime i prezime','Pozicija','Status'], rows:state.employees.map((employee)=>[employee.name,employee.role,attendance[employee.id]!==false?'PRISUTAN':'ODSUTAN']) })
      await renderWorkforceArchive('attendance')
    } catch { alert('PDF trenutno nije moguće napraviti.') }
    finally { button.disabled=false; button.textContent='Sačuvaj PDF' }
  })
  ...
  document.querySelector('.attendance-list').addEventListener('click', (event) => {
    ...
    const isPresent = button.dataset.attendance === 'active'
    attendance[employeeId] = isPresent
    if (!state.workHours[dateKey]) state.workHours[dateKey] = {}
    if (isPresent) {
      if (!(Number(state.workHours[dateKey][employeeId]) > 0)) state.workHours[dateKey][employeeId] = 9
    } else {
      state.workHours[dateKey][employeeId] = 0
    }
    saveAttendance()
    saveWorkHours()
...
function workHoursFor(dateKey) {
  ...
  state.employees.forEach((employee) => {
    if (attendance[employee.id] === false) state.workHours[dateKey][employee.id] = 0
    else if (state.workHours[dateKey][employee.id] === undefined) state.workHours[dateKey][employee.id] = 9
  })
...
function workHoursPage(dateKey = dateKeyFor()) {
  ...
  document.querySelector('#hours-form').addEventListener('submit', async (event) => {
    ...
    if (submitButton) { submitButton.disabled=true; submitButton.textContent='Čuvam sate i PDF…' }
    const form = new FormData(event.currentTarget)
    ...
    try {
      await createWorkforcePdf({ type:'hours', title:'DNEVNI RADNI SATI', periodLabel:dateKey.split('-').reverse().join('.'), filePrefix:`Radni-sati-${dateKey}`, columns:['Ime i prezime','Pozicija','Radni sati'], rows:state.employees.map((employee)=>[employee.name,employee.role,`${hours[employee.id] || 0} h`]) })
    } catch { alert('Sati su sačuvani, ali PDF trenutno nije napravljen.') }
...
function monthlyHoursPage(monthKey = dateKeyFor().slice(0, 7)) {
  ...
  content.innerHTML = employeeHubTabs('monthly-hours') + `<section class="page-heading monthly-heading compact-employee-heading"><div><p class="eyebrow">Evidencija rada</p><h1>Mesecni pregled sati</h1><p>Pregled sacuvanih radnih sati po zaposlenom.</p></div><label class="attendance-date"><span>Mesec</span><input id="monthly-hours-date" type="month" value="${monthKey}"></label></section><section class="monthly-stats"><article><span>Ukupno sati</span><strong>${allHours.toLocaleString('sr-RS')}</strong><small>za ceo tim</small></article><article><span>Broj zaposlenih</span><strong>${state.employees.length}</strong><small>u evidenciji</small></article><article><span>Mesec</span><strong>${daysInMonth}</strong><small>kalendarskih dana</small></article></section><section class="monthly-panel"><header><div><h2>${monthTitle.charAt(0).toLocaleUpperCase('sr')}${monthTitle.slice(1)}</h2><p>Radni dani se racunaju samo kada zaposleni ima vise od 0 upisanih sati.</p></div></header><div class="monthly-table"><div class="monthly-head"><span>Zaposleni</span><span>Ukupno sati</span><span>Radnih dana</span><span>Neradnih dana</span></div>${totals.length ? totals.map(({ employee, total, workDays, nonWorkDays }) => `<div class="monthly-row"><span><b>${esc(employee.name)}</b><small>${esc(employee.role)}</small></span><strong>${total.toLocaleString('sr-RS')} h</strong><em class="work-days">${workDays}</em><em class="non-work-days">${nonWorkDays}</em></div>`).join('') : '<p class="plan-empty">Nema zaposlenih u evidenciji.</p>'}</div></section><section class="workforce-pdf-create"><div><h2>PDF mesečnog obračuna</h2><p>Izvezite sate, radne dane i neradne dane svih radnika za izabrani mesec.</p></div><button type="button" class="primary-btn" id="save-monthly-pdf">Sačuvaj mesečni PDF</button></section>${workforceArchiveSection('monthly','Sačuvani PDF mesečni izveštaji')}`
  renderWorkforceArchive('monthly')
  document.querySelector('#save-monthly-pdf').addEventListener('click', async (event) => {
    const button=event.currentTarget; button.disabled=true; button.textContent='Pravim PDF…'
    try {
      await createWorkforcePdf({ type:'monthly', title:'MESEČNI IZVEŠTAJ RADNIH SATI', periodLabel:monthTitle, filePrefix:`Mesecni-sati-${monthKey}`, columns:['Ime i prezime','Pozicija','Ukupno sati','Radnih dana','Neradnih dana'], rows:totals.map(({employee,total,workDays,nonWorkDays})=>[employee.name,employee.role,`${total} h`,String(workDays),String(nonWorkDays)]) })
      await renderWorkforceArchive('monthly')
    } catch { alert('PDF trenutno nije moguće napraviti.') }
    finally { button.disabled=false; button.textContent='Sačuvaj mesečni PDF' }
  })
...
function renderExpandedControlCenter() {
  ...
  root.innerHTML = `
    ...
    <article class="control-detail-section people-detail-section">
      <header><div><span>●</span><div><p class="eyebrow">PRISUTNOST I LJUDI</p><h2>Kompletno brojno stanje</h2><small>Ko je prisutan, ko nije i mesečni izostanci.</small></div></div><b>${activeEmployees.filter((employee)=>todayAttendance[employee.id]!==false).length}/${state.employees.length}<small>prisutno / ukupno</small></b></header>
      <div class="control-people-full">${state.employees.map((employee) => {
        const enabled=true, present=todayAttendance[employee.id]!==false
...
function controlCenterPage() {
  ...
  content.innerHTML = `<section class="control-center">
    ...
    <section class="control-kpis">
      ...
      <button data-control-go="employees" class="green"><span>●</span><p>Prisutni danas</p><strong>${presentToday.length}</strong><small>${absentToday.length} odsutnih</small></button>
    ...
    <section class="control-main-grid">
      ...
      <article class="control-panel control-people"><header><div><p class="eyebrow">LJUDI</p><h2>Prisutnost 7 dana</h2></div><b>${presentToday.length}/${state.employees.length}<small>prisutno / ukupno</small></b></header><div class="control-week">${weekDays.map((day) => `<div><b>${day.value ?? '—'}</b><span><i style="height:${day.value == null ? 4 : Math.max(8,day.value/maxPeople*100)}%"></i></span><small>${day.label}</small></div>`).join('')}</div><footer><span>Ovaj mesec: <b>${monthPresent}</b> prisustva</span><span>Izostanci: <b>${monthAbsent}</b></span></footer><button data-control-go="employees">Otvori evidenciju →</button></article>
    </section>
