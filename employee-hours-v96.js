...
(function () {
  ...
  document.addEventListener('click',event=>{
    ...
    if((b=target.closest('[data-eh-export]'))){try{const raw=localStorage.getItem(KEY);if(raw)state=M.validate(JSON.parse(raw))}catch(e){el('eh-export-status').textContent='Izvoz nije moguć: '+e.message;return}if(!exportState().employees.length)return;document.dispatchEvent(new CustomEvent('tasker-hours-export',{detail:{state:exportState(),employeeId:selectedEmployeeId,action:b.dataset.ehExport,format:el('eh-export-format').value}}));return}
