(function (root) {
  ...
  const defaultSites=()=>[{id:'bajkmont',name:'BAJKMONT / VERTIV'},{id:'monting',name:'MONTING / VERTIV'},{id:'rugvica',name:'RUGVICA / VERTIV'}]
  const sites=s=>s.sites||defaultSites()
  const assignment=(s,id,date=s.date)=>s.days[date]?.assignments?.[id]||''
  function validate(s){
    ...
    for(const site of sites(s)){if(!site.id||site.id==='unassigned'||siteIds.has(site.id)||typeof site.name!=='string'||!site.name.trim())throw Error('Neispravno gradilište.');siteIds.add(site.id)}
    for(const d of Object.values(s.days))for(const id of Object.values(d.assignments||{}))if(!siteIds.has(id))throw Error('Nepoznato gradilište u rasporedu.')
    return s
  }
  ...
  function change(s,action){
    ...
    switch(action.type){
      ...
      case 'addSite':{
        ...
        if(!name||!action.id)throw Error('Upišite naziv gradilišta.')
        n.sites=sites(n)
        if(n.sites.some(x=>x.id===action.id||x.name.toLocaleLowerCase()===name.toLocaleLowerCase()))throw Error('Gradilište već postoji.')
        n.sites.push({id:action.id,name});break
      }
      case 'assignHistory':{
        if(!validDate(action.until)||!sites(n).some(x=>x.id===action.siteId))throw Error('Neispravno gradilište ili datum.')
        for(const [date,d] of Object.entries(n.days)){
          if(date>action.until)continue
          for(const e of n.employees){
            if(!active(e,date)||!d.statuses[e.id]||d.assignments?.[e.id])continue
            d.assignments=d.assignments||{};d.assignments[e.id]=action.siteId
          }
        }
        break
      }
      case 'assign':{
        if(action.siteId&&!sites(n).some(x=>x.id===action.siteId))throw Error('Odaberite gradilište.')
        if(!Array.isArray(action.ids)||!action.ids.length)throw Error('Označite zaposlene.')
        for(const id of action.ids){
          if(!n.employees.some(e=>e.id===id&&active(e,n.date)))throw Error('Zaposleni nije aktivan na taj datum.')
          const d=ensureDay(n);d.assignments=d.assignments||{}
          if(action.siteId)d.assignments[id]=action.siteId;else delete d.assignments[id]
        }
        break
      }
  ...
  function siteState(s,siteId){
    const n=JSON.parse(JSON.stringify(s))
    if(!siteId)return n
    const site=sites(s).find(x=>x.id===siteId)
    if(!site&&siteId!=='unassigned')throw Error('Nepoznato gradilište.')
    n.site=site?site.name:'Neraspoređeno'
    const matches=(id,date)=>assignment(s,id,date)===(siteId==='unassigned'?'':siteId)
    n.employees=n.employees.filter(e=>Object.entries(s.days).some(([date,d])=>date.startsWith(s.month+'-')&&active(e,date)&&matches(e.id,date)&&(siteId!=='unassigned'||d.statuses[e.id])))
    const ids=new Set(n.employees.map(e=>e.id))
    for(const [date,d] of Object.entries(n.days))for(const key of ['statuses','overrides','assignments'])if(d[key])d[key]=Object.fromEntries(Object.entries(d[key]).filter(([id])=>ids.has(id)&&matches(id,date)))
    return n
  }
  const api={sites,assignment,siteState,active,today,create,validate,day,minutes,change,month}
